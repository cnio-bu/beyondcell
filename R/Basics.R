...
#' \item{\code{preferred.and.sigs}}: \code{Preferred_Name} (or alternatively
#' \code{bc.names}) and \code{sig_id}.
...
FindDrugs <- function(bc, x, na.rm = TRUE) {
  ...
  # Add bc.names column and remove names that are not sig IDs from sig_id c
  # olumn.
  df$bc.names <- df$IDs
